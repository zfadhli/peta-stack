...
export async function run(): Promise<void> {
  ...
  cli
    .command("migrate:diff", "Preview schema changes without writing a migration")
    .action(async () => {
      const config = await loadConfig()
      const snapshotPath = resolve(config.migrationsDir, "snapshot.json")
      const prevSnapshot = loadSnapshot(snapshotPath)
  ...
  cli.command("migrate:up", "Apply pending migrations").action(async () => {
    ...
    for (const m of migrations) {
      ...
      if (!verifyChecksum(config.migrationsDir, m.name, filePath)) {
        console.error(
          `❌ Checksum mismatch for migration "${m.name}". File has been modified since creation.`,
        )
        process.exit(1)
      }
    }
  ...
  cli
    .command("migrate:down [steps]", "Rollback migrations (default: 1)")
  ...
  cli.command("migrate:status", "Show migration status").action(async () => {
    const config = await loadConfig()
    const migrations = await loadMigrationFiles(config.migrationsDir)
    const { completed, pending } = await createMigrationRunner(config.getKysely()).status(
      migrations,
    )
    ...
    if (name) {
      ...
      for (const file of seedFiles) {
        try {
          ...
          console.error(`❌ Seed "${file}" failed:`, (err as Error).message)
          failed++
        }
      }
      if (failed === 0) {
        spinner.succeed(`Executed ${passed} seed(s)`)
      } else {
        spinner.fail(`Executed ${passed} seed(s), ${failed} failed`)
      }
    }
  })
