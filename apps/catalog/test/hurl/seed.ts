...
const { User, Author, Category, Book, BookCategory } = await import("../../src/db/schema.js")
...
const authorUser = await User.insert({ email: "author@hurl.test", passwordHash: ph, name: "Author User", role: "author" })
const user = await User.insert({ email: "user@hurl.test",   passwordHash: ph, name: "Regular User", role: "user" })

// ── Author profiles ───────────────────────────────────────────────────
// Linked to author user (used for owner tests)
const linkedAuthor = await Author.insert({ name: "Hurl Author", bio: "Hurl test author", userId: authorUser.get<string>("id") })
