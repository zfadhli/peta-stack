...
app.get("/",
  route().summary("List all categories").tags("categories").response(200, CategoryResponse.array())
    .handle(async (c) => {
...
app.post("/",
  requireSession(),
  route().summary("Create a new category").tags("categories")
    .requestBody(CreateCategoryBody).response(201, CategoryResponse)
    .response(401, "Unauthorized").response(409, "Category already exists")
    .handle(async (c) => {
