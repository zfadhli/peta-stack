...
const AuthorListResponse = type({ data: AuthorResponse.array(), total: "number", perPage: "number", currentPage: "number", lastPage: "number", hasMorePages: "boolean" })
const CreateAuthorBody = type({ name: "string>0", bio: "string?" })

app.get("/",
  route().summary("List authors").tags("authors").paginated({ maxLimit: 50 })
    ...
    .handle(async (c) => {
      ...
      return c.json({
        ...
        total: json.total, perPage: json.perPage, currentPage: json.currentPage,
        lastPage: json.lastPage, hasMorePages: json.hasMorePages,
      })
    }),
)
...
app.post("/",
  requireSession(),
  route().summary("Create a new author").tags("authors")
    .requestBody(CreateAuthorBody).response(201, AuthorResponse).response(401, "Unauthorized")
    .handle(async (c) => {
